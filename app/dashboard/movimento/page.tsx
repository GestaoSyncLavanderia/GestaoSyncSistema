...
function MovimentoContent() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`/api/movimento?from=${from}&to=${to}`, { cache: "no-store" }).then((r) => r.ok ? r.json() : null),
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
          ...
          {([["relatorios", BarChart2, "Relatórios"], ["unidades", LayoutGrid, "Unidades"]] as const).map(([key, Icon, label]) => (
            <button
              ...
              className={cn(
                ...
                view === key
                  ? "bg-white text-[#F59E0B] shadow-sm border border-[#E5E7EB]"
        ...
        <div className="flex items-center gap-2 rounded-full bg-amber-50 border border-amber-100 px-3 py-1.5">
          <MonitorSmartphone size={13} className="text-amber-400 shrink-0" />
          <span className="text-xs text-amber-600">
            Espelha a aba <span className="font-semibold">Vendas</span> do SisLav
          </span>
        </div>
      </div>
      ...
      {view === "relatorios" ? (
        <>
          ...
          <div className="grid grid-cols-3 gap-3">
            <KpiBlock icon={TrendingUp} label="Movimento bruto" color="#F59E0B"
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="col-span-2 rounded-xl border border-[#E5E7EB] bg-white p-4">
              <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-3">
                {chartData.length <= 1 ? "Movimento por unidade — hoje" : "Evolução diária"}
              </p>
              {chartData.length <= 1 ? (
                  ...
                  {top5.map((unit, i) => (
                    <div key={unit.laundryId} className="flex items-center justify-between gap-2 py-2 border-b border-[#F3F4F6] last:border-0">
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-sm font-semibold text-[#F59E0B]">{formatCurrency(unit.total)}</p>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="rounded-xl border border-[#E5E7EB] bg-white p-4">
              <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-3">Movimento por estado</p>
              {(() => {
                    ...
                    {byState.map((row) => (
                      <div key={row.state}>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-[11px] font-semibold text-[#F59E0B] ml-2 shrink-0">{formatCurrency(row.total)}</span>
                        </div>
                        <div className="h-1.5 w-full rounded-full bg-[#F3F4F6]">
                          <div className="h-full rounded-full bg-[#F59E0B] transition-all duration-500" style={{ width: `${(row.total / maxTotal) * 100}%` }} />
                        </div>
                      </div>
                    ))}
                  </div>
